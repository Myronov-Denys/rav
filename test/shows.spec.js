...
test('Open "Show Schedule" shows from "WATCH RAV TV" section on home page', async ({ page }) => {
    const homePage = new HomePage(page);
    const showsPage = new ShowsPage(page);

    console.log('Step 1: Scroll down to the "WATCH RAV TV" section');
    await homePage.viewShowScheduleButtonOnWatchRavTvSection.scrollIntoViewIfNeeded();

    console.log('Step 2: Click the "View Show Schedule" button');
    await homePage.viewShowScheduleButtonOnWatchRavTvSection.click();

    console.log('Assert test 1: The "Show Schedule" shows title is displayed');
    await expect(showsPage.showScheduleShowsTitile).toBeVisible();
});

test('Open "Show Schedule" page from "All Shows" page ', async ({ page }) => {
    const homePage = new HomePage(page);
    const showsPage = new ShowsPage(page);

    console.log('Step 1: Open the "Shows" page');
    await homePage.showsHeaderButton.click();

    console.log('Step 2: Scroll down to the "View Show Schedule" button');
    await showsPage.viewShowScheduleButtonOnAllShowsPage.scrollIntoViewIfNeeded();

    console.log('Step 3: Click the "View Show Schedule" button');
    await showsPage.viewShowScheduleButtonOnAllShowsPage.click();
